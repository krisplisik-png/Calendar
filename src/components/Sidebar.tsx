    ...
    <div className="sidebar-section">
      ...
      <div className="group-list">
        {groups.map(group => <div className="group-filter" key={group.id}>
          ...
          <button className="participant-name" onClick={() => setPendingDelete(group)} title="Удалить запись"><span>{group.name}<small className="kind-label">{(group.kind ?? 'group') === 'group' ? 'группа' : group.kind === 'pair' ? 'пара' : 'индивидуально'}</small></span><Trash2 size={14} /></button>
        </div>)}
