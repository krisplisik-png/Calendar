...
export interface GroupInput { name: string; kind: GroupKind; color: string; course: string; level: string; notes: string; monthlyLessonTarget: 6 | 7 | 8 | 9 | 10 }

export function GroupDialog({ onClose, onSave }: { onClose: () => void; onSave: (value: GroupInput) => Promise<void> }) {
  const [value, setValue] = useState<GroupInput>({ name: '', kind: 'group', color: '#a98be8', course: '', level: '', notes: '', monthlyLessonTarget: 8 });
