...
export function LessonDialog({ groups, lesson, occurrenceDate, initialDate, teacherMode = false, onClose, onSave, onDelete }: {
  ...
  return <div className="dialog-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <form className="dialog lesson-dialog" onSubmit={submit}>
      ...
      <div className="form-grid three"><label>{selectedRecurringOccurrence ? 'Дата выбранного занятия' : 'Дата'}<input type="date" value={displayedDate} onChange={e => setValue({ ...value, date: e.target.value })} required disabled={teacherMode || selectedRecurringOccurrence} /></label><label>Начало<input type="time" value={value.startTime} onChange={e => setValue({ ...value, startTime: e.target.value })} required disabled={teacherMode} /></label><label>Конец<input type="time" value={value.endTime} onChange={e => setValue({ ...value, endTime: e.target.value })} required disabled={teacherMode} /></label></div>
