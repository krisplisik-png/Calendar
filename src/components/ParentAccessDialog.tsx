...
export function ParentAccessDialog({ students, groups, lessons, access, syncing, syncError, onCreateStudent, onCreateLink, onCreateMissingLink, onRename, onPrepare, onRegenerate, onDisable, onRebuild, onClose }: {
  ...
  async function copyPrepared(item: ParentAccess) {
  ...
  async function openAndRepair(item: ParentAccess) {
    const tab = window.open('', '_blank');
    if (tab) { tab.document.title = 'Подготавливаем расписание'; tab.document.body.textContent = 'Подготавливаем расписание…'; }
    setBusy(true); setOperationError('');
    try {
