...
export function ParentAccessDialog({ students, groups, access, syncing, syncError, onCreateStudent, onCreateLink, onRegenerate, onDisable, onRebuild, onClose }: {
  students: Student[]; groups: Group[]; access: ParentAccess[]; syncing?: boolean; syncError?: string;
  ...
  return <div className="dialog-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}><section className="dialog parent-access-dialog"><header><div><p className="eyebrow">РОДИТЕЛЬСКИЙ ДОСТУП</p><h2>Ученики и семейные ссылки</h2></div><button onClick={onClose}><X /></button></header>
    ...
    <section className="parent-links"><h3>Созданные ссылки</h3><label className="parent-link-search"><Search size={17} /><input type="search" value={linkSearch} onChange={event => setLinkSearch(event.target.value)} placeholder="Найти ребенка по имени или фамилии" aria-label="Поиск ребенка в созданных ссылках" />{linkSearch && <button type="button" onClick={() => setLinkSearch('')} aria-label="Очистить поиск"><X size={15} /></button>}</label>{filteredAccess.length ? filteredAccess.map(item => <article key={item.id}><div><strong>{item.studentIds.map(id => studentMap.get(id)?.fullName).filter(Boolean).join(' · ') || 'Ученик не найден'}</strong><small>{item.active ? 'Доступ активен' : 'Доступ отключён'}</small></div>{item.active && <><button title="Скопировать" onClick={() => copy(item.token)}><Copy size={16} /></button><button title="Открыть" onClick={() => window.open(urlFor(item.token), '_blank', 'noopener,noreferrer')}><ExternalLink size={16} /></button><button title="Создать новую ссылку" onClick={async () => { setBusy(true); try { await copy(await onRegenerate(item)); } finally { setBusy(false); } }}><RefreshCw size={16} /></button><button className="link-danger" title="Отключить" onClick={() => onDisable(item)}><Unlink size={16} /></button></>}</article>) : <p className="journal-empty">{access.length ? 'Ребенок с таким именем не найден.' : 'Родительских ссылок пока нет.'}</p>}</section>
