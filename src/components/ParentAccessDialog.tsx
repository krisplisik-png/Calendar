...
export function ParentAccessDialog({ students, groups, lessons, access, syncing, syncError, onCreateStudent, onCreateLink, onCreateMissingLink, onPrepare, onRegenerate, onDisable, onRebuild, onClose }: {
  ...
  async function prepareAndCopy(item: ParentAccess) {
    setBusy(true); setOperationError('');
    try { await onPrepare(item); await copy(item.token); }
    catch (error) { setOperationError(error instanceof Error ? error.message : String(error)); }
    finally { setBusy(false); }
  }
  async function prepareAndOpen(item: ParentAccess) {
    const tab = window.open('', '_blank');
    if (tab) { tab.opener = null; tab.document.body.textContent = 'Подготавливаем расписание…'; }
    setBusy(true); setOperationError('');
    try {
      await onPrepare(item);
      if (tab) tab.location.replace(urlFor(item.token)); else window.open(urlFor(item.token), '_blank', 'noopener,noreferrer');
    } catch (error) {
      if (tab) tab.close();
