...
export function GroupParentPage({ groupId, studentId, studentName, groupName }: { groupId: string; studentId: string; studentName: string; groupName: string }) {
  ...
  const homeworkSummary = occurrences.reduce((summary, item) => {
    const itemFeedback = feedbackByOccurrence[`${item.lesson.id}__${item.occurrenceDate}`];
    if (itemFeedback?.homeworkAssigned === true) {
      summary.total += 1;
      if (itemFeedback.homeworkDone) summary.done += 1;
    }
    return summary;
  }, { done: 0, total: 0 });
